#*[-----------------------------------------------------------------------------------------------]*#
#*[ Objectives : This program applies our changepoint method to COVID-19 daily confirmed cases    ]*#
...
### Create dataset with common first and last dates (2020-03-05 to 2021-03-03)
date.first_target  <- "2020-03-05"
...
GA.sol <- foreach (i=1:n.states,.packages='tidyverse') %dopar% {
  ...
  t.1_i <- as.Date(data_i$date[t.1_i.loc])               # first day for 7-day moving-average: 2020-03-08
  ...
  x.t_all <- c(data_i$cases[1],diff(data_i$cases))       # daily count of new cases in data
  ...
  y.t <- data_i$cases[t.1_i.loc:t.n_i.loc]               # daily count of cumulative cases
  x.t <- x.t_all.mis[t.1_i.loc:t.n_i.loc]                # daily count of new cases, with negative as missing
  m.t_0 <- m.t_all[t.1_i.loc:t.n_i.loc]                  # 2q+1=7 day moving-average
  m.t <- ifelse(m.t_0>=0.5,m.t_0,0.5)                    # if m.t_0 < 0.5, then m.t is set to equal 0.5
  ...
  ga.out <- ga.cpt_ts(y=lm.t,fitness=pnllik.MDL_RWD,gen.size=200,max.itr=150,p.mut=0.05,
                      seed=10*(i-1)+543,is.graphic=FALSE,is.print=FALSE,is.export=FALSE)
...
capture.output(GA.sol,file=paste(WD.out,"out_ga-RWD_cases_",t.n_i,".txt",sep=""))
save(GA.sol,file=paste(WD.out,"out_ga-RWD_cases_",t.n_i,".RData",sep=""))
...
t.1_i <- "2020-03-08"                                    # first day for 7-day moving-average: 3-(05+3)
t.n_i <- "2021-02-28"                                    # last day for 7-day moving-average: 3-(03-3)

load(file=paste(WD.out,"out_ga-RWD_cases_",t.n_i,".RData",sep="")) # Loaded is GA.sol
GA.sol
...
write.csv(GA.result,file=paste(WD.out,"out_ga-RWD_cases_",t.n_i,".csv",sep=""))
